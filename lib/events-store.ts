...
const seedBase = [
  ["Palmerston North NRoB", "2026-03-01", "Events Completed", "completed", 180, 53],
  ...
  ["Melbourne", "2026-05-11", "AYB Tour 2026 (AU - May Events)", "upcoming", 244, 0],
  ["Adelaide", "2026-05-13", "AYB Tour 2026 (AU - May Events)", "upcoming", 141, 0],
  ["Perth", "2026-05-15", "AYB Tour 2026 (AU - May Events)", "upcoming", 190, 0],
  ["Brisbane (May)", "2026-05-19", "AYB Tour 2026 (AU - May Events)", "upcoming", 86, 0],
  ["Sydney (May)", "2026-05-22", "AYB Tour 2026 (AU - May Events)", "upcoming", 109, 0],
...
export function getSeedData(): EventItem[] {
  return seedBase.map(([name, date, group, status, registrations, attendees]) => ({
    id: createId(),
    name,
    date,
    group,
...
export function loadEvents() {
  ...
  try {
    ...
    return Array.isArray(parsed) ? (parsed as EventItem[]) : getSeedData();
  } catch {
    return getSeedData();
  }
}
